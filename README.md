# XiangYun Image Studio

XiangYun Image Studio（向云图像工坊 / 向云Pro）是一款开源的 AI 图像生成与编辑桌面工具，基于 Electron、React 和 TypeScript 构建。它面向电商、短剧、自媒体、设计团队和商业内容生产场景，支持文生图、图生图、批量生产、行业模板、素材管理、AI 修图工具箱和云端存储。
...
```text
https://github.com/h614626370-del/XiangYun-Image-Studio/releases/tag/v0.1.10
```

客户端文件：

- Windows x64 便携版：`XiangYun-Image-Studio-0.1.10-win-x64-portable.exe`
...
- 固定接口：API Base URL 固定为 `https://kkflow.org`。
- 本机安全存储：API Key 和云存储密钥只保存在本机安全存储中。
- 跨平台配置：同一套代码支持 Windows 和 macOS 构建。

## v0.1.10 更新重点

- 新增年轻化行业模板：`人物图像`、`二次元头像`、`潮流社交`、`虚拟IP`。
- `人物图像` 覆盖社交头像写真、AI 写真大片、城市街拍、年轻职场形象照、国风汉服写真、情侣头像、校园青春毕业照、音乐人宣传照等。
- `二次元头像` 覆盖日系清透头像、酷感少年少女头像、Q 版萌系头像、二次元情侣头像、动漫手机壁纸、二次元表情包、偶像应援头像等。
- `潮流社交` 覆盖 Y2K 潮流海报、多巴胺封面、赛博朋克头像、独立音乐封面、社交热梗表情图、手机锁屏壁纸、穿搭灵感板等。
- `虚拟IP` 覆盖虚拟主播头像、虚拟人设定稿、直播间虚拟背景、虚拟 IP 表情包、品牌吉祥物 IP、IP 盲盒概念图、角色资料卡等。
- 扩充自媒体内容、漫画动漫、游戏美术、潮玩玩具、个人 IP 等热门方向模板数量。
- 更新中文行业模板文档和模板种子配置，便于阅读、导入和二次开发。
- 重新构建并发布 Windows、macOS Intel、macOS Apple Silicon 三个客户端。
